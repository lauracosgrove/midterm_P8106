...
x2 <- model.matrix(heart_disease_mortality_per_100k ~ ., data = heart2)[, -1] # get rid of factor vairables / work with smaller dataset so R doesn't crash
y2 <- heart2$heart_disease_mortality_per_100k
...
```{r}
saveRDS(step.fit, 'lm_step.rds')
saveRDS(gam.fit, 'gam_fit.rds')
```
